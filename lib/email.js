...
export async function sendCheckWatchAlertEmail(
  email,
  { eventLabel, reason, statusLabel, watchId }
) {
  const base = appUrl();
  return sendTemplated(
    email,
    ...
    { devExtra: { eventLabel, reason, statusLabel, watchId } }
  );
}

export async function sendAccountDeletionEmail(email, token) {
  const base = appUrl();
  const link = `${base}/api/profile/delete-account?token=${token}`;
  return sendTemplated(email, accountDeletionTemplate({ appUrl: base, link }), {
    devExtra: { link },
  });
}

export async function sendEventReminderEmail(email, { eventLabel, daysLeft }) {
  const base = appUrl();
  return sendTemplated(
    email,
    eventReminderTemplate({ appUrl: base, eventLabel, daysLeft }),
    { devExtra: { eventLabel, daysLeft } }
  );
}

...
export async function sendCheckWatchAlertEmail(email, { eventLabel, reason, statusLabel }) {
  const resend = getResend();
  const link = `${appUrl()}/check`;

  const subject = "Alerte billet — VerifyMyTicket";
  const html = `
    <p>Une activité a été détectée sur une vérification que vous suivez.</p>
    <p><strong>Événement :</strong> ${eventLabel || "Non précisé"}</p>
    <p><strong>Motif :</strong> ${reason}</p>
    ${statusLabel ? `<p><strong>État signalé :</strong> ${statusLabel}</p>` : ""}
    <p><a href="${link}">Retourner à la vérification</a></p>
    <p style="color:#666;font-size:12px">Vous recevez cet email car vous avez laissé votre adresse lors d'une vérification sur VerifyMyTicket.</p>
  `;

  if (!resend) {
    console.info("[email:dev] Alerte vérif:", email, { eventLabel, reason, statusLabel });
    return { ok: true, dev: true };
  }

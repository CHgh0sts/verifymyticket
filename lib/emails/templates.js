...
export function checkWatchAlertTemplate({
  ...
  const alertsUrl = watchId
    ? `${appUrl}/dashboard/alerts?watch=${watchId}`
  ...
  const bodyHtml = [
    ...
    primaryButton(alertsUrl, "Voir mes alertes"),
    mutedParagraph(
...
export function accountDeletionTemplate({ appUrl, link }) {
  const title = "Confirmer la suppression du compte";
  const bodyHtml = [
    paragraph(
      "Vous avez demandé la suppression définitive de votre compte VerifyMyTicket."
    ),
    warningBox(
      "Cette action est irréversible : billets, veilles et clés API organisateur seront effacés."
    ),
    primaryButton(link, "Confirmer la suppression"),
    mutedParagraph("Ce lien expire dans 1 heure. Si vous n’êtes pas à l’origine de cette demande, ignorez cet email."),
  ].join("");

  return {
    subject: "Suppression de compte — VerifyMyTicket",
    html: renderEmailLayout({
      title,
...
export function eventReminderTemplate({ appUrl, eventLabel, daysLeft }) {
  const title = daysLeft <= 1 ? "Votre événement est demain" : `J-${daysLeft} avant votre événement`;
  const bodyHtml = [
    paragraph(
      daysLeft <= 1
        ? `Rappel : ${escapeHtml(eventLabel)} a lieu bientôt.`
        : `Rappel : ${escapeHtml(eventLabel)} dans ${daysLeft} jours.`
    ),
    paragraph(
      "Vérifiez une dernière fois que votre billet n’a pas été signalé comme recyclé, et gardez vos alertes actives."
    ),
    primaryButton(`${appUrl}/dashboard/events`, "Voir mes événements"),
  ].join("");

  return {
    subject: `${title} — VerifyMyTicket`,
    html: renderEmailLayout({
      title,

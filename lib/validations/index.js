...
export const publicCheckSchema = z
  .object({
    intent: z.enum(["owned", "precheck"]).default("precheck"),
    checkType: z.enum(["name", "seat", "barcode"]).default("barcode"),
    mode: z.enum(["full", "last4"]).default("full"),
    barcodeValue: z.string().max(4096).optional(),
    purchaserName: z.string().max(200).optional(),
    block: z.string().max(50).optional(),
    row: z.string().max(50).optional(),
    seat: z.string().max(50).optional(),
    alertEmail: z.string().email("Email invalide").max(255).optional().nullable(),
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.checkType === "name") {
      if (!String(data.purchaserName || "").trim() || String(data.purchaserName).trim().length < 2) {
        ctx.addIssue({
          code: "custom",
          path: ["purchaserName"],
          message: "Indiquez le nom et prénom figurant sur le billet",
        });
      }
      return;
    }

    if (data.checkType === "seat") {
      ...
      if (!block && !row && !seat) {
        ctx.addIssue({
          code: "custom",
          path: ["seat"],
          message: "Indiquez au moins un élément d'emplacement (bloc, rang ou siège)",
        });
      }
      return;
    }
...
export const confirmPurchaseSchema = z.object({
  ...
  purchased: z.boolean(),
  alertEmail: z.string().email().max(255).optional().nullable(),
});

export { ticketUpdateSchema } from "./ticket-update";
